// js/tools/EqualTool.js — Select two segments to make equal length
...
export class EqualTool extends BaseTool {
  ...
  onClick(wx, wy) {
    ...
    if (this.step === 0) {
      ...
      takeSnapshot();
      state.scene.addConstraint(new EqualLength(this._firstSeg, hit));
      state.emit('change');
      this._firstSeg = null;
      this.step = 0;
      this.setStatus('Equal length applied. Click first segment for next, or switch tool.');
    }
  }
