...
for (const face of geometry.topoBody.faces()) {
  ...
  console.log('face', face.id, face.surfaceType, face.shared, 'coedges', face.outerLoop.coedges.length);
  for (const coedge of face.outerLoop.coedges) {
    ...
    console.log(' ', edge.id, coedge.sameSense, edgeVKey(a), '->', edgeVKey(b), edge.curve?.degree, edge.curve?.controlPoints?.length);
  }
}
